...
from uis.scriptsWindow02_UI import Ui_Form
...
class scriptsWindow(QtWidgets.QWidget, Ui_Form):
...
#         super(scriptsWindow, self).__init__()
        ...
        buttons = [self.flopflop, self.autocrop, self.viewerToRGBA, self.deleteWriteFiles, self.rerenderMissingFrames, self.lineupHorizontally, self.lineupVertically, self.toggleViewerInputs,
                  self.mirrorNodes, self.w_scaletree, self.glezin, self.batchRenamer, self.deepDefocusSlicer, self.normalize, self.copyTrackingDataToRoto, self.cameraFromExr]
        for b in buttons:
