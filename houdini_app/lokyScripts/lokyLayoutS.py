...
def makeDailise():
	...
	cfg = hou.ui.readInput("Write wiewwrite config, or SKIP", buttons=('OK',), severity=hou.severityType.Message)[1]
	if len(cfg)>0:
		hou.hscript("viewwrite -f $FSTART $FEND -r 1920 1080 "+cfg+" -c -R current -q 3 Build.panetab1.world.persp1 " + "'"+sequence_path+"/dailies.$F4.jpg'")
	else:
		hou.hscript("viewwrite -f $FSTART $FEND -r 1920 1080 -c -R current -q 3 Build.panetab1.world.persp1 " + "'"+sequence_path+"/dailies.$F4.jpg'")
	...
	for s in seqs:
		...
		# SOUND #
		soundFolder = os.environ["SHOT"] + "/sound/".replace("\\", "/")
		print "Sound folder: ", soundFolder
		if os.path.exists(soundFolder) and len(filter(os.path.isfile, glob.glob(soundFolder + "*.wav"))) > 0:
		...
		if lastSoundFile:
			cmd = mpg + " -threads 8 -r 24 -i " + lastSoundFile + " -start_number " + s.format('%s') + " -i " + sq + " -threads 8 -y -framerate 24 -c:v libx264 -pix_fmt yuv420p -preset ultrafast -crf 23 " + out_path + "/"+out_file
		else:
			cmd = mpg + " -threads 8 -r 24 -start_number " + s.format(
				'%s') + " -i " + sq + " -threads 8 -y -framerate 24 -c:v libx264 -pix_fmt yuv420p -preset ultrafast -crf 23 " + out_path + "/" + out_file
	...
	for index in selectedIndex:
		...
		for node in allnodes:
			...
			if context=="Object":
				...
				if nodeType == "geo" or nodeType == "instance":
		...
		hou.parm(ropNode.path()+"/packed_transform").set("transform")
...
def createShader(operator=None, node_out=None):
		### color ###
	...
	if node_out:
		materialNode.setInput(0, node_out[0], 0)
...
def geoToFileCache():
	...
	for selNode in nodes:
		allnodes = allnodes+selNode.allSubChildren(top_down=True, recurse_in_locked_nodes=False)
	for node in allnodes:
		if node.type().name().split("::")[0] == "geo":
			print node
			fileExt = hou.parm(node.path()+"/file_extension").eval()
			nodePath = node.path()
			parent = node.parent()
			...
			if fileExt!=2:
				...
				filecache = parent.createNode("alembic", "cache_"+objectname+"_"+component, run_init_scripts=True)
				hou.parm(filecache.path()+"/fileName").set(file)
				hou.parm(filecache.path()+"/groupnames").set("xformbase")
			if inputCount!=0:
				filecache.setInput(0, intput, 0)
			for output in outputs:
				outputImputs = output.inputs()
				for counter, targetNode in enumerate(outputImputs):
	...
	for folder in folderList:
		...
		for file in files:
			...
			if (ext[1] == ".exr") or (ext[1] == ".png") or (ext[1] == ".jpg") or (ext[1] == ".tif") or (ext[1] == ".jpeg"):
				...
				cmd = 'X:/app/win/ffmpeg/bin/ffmpeg -i '+folder+'/'+file+' -n -s 512x512 '+proxyFolder+'/'+ext[0]+'.jpg'
				sp.call(cmd, shell=True)
				index=index+1
