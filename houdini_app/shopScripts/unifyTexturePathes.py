...
def createEmptyTp(parent_shop):
    ...
    # MASTERPATH
    masterpathP = hou.StringParmTemplate(
        'master_path',
        'Master Path',
        1,
        join_with_next=True,
    ...
    linkMasterpathP = hou.ButtonParmTemplate(
        "link_master_substring",
        "Link",
        join_with_next=False,
        help='Находит мастерпасс строку в путях к текстурам и заменяет путь папки на ссылку к мастерпасу',
    ...
    # BUTTONS SECTION
    createEmptyShaderP = hou.ButtonParmTemplate(
        "create_images",
        "Create Empty Shader",
        join_with_next=True,
        help='Создает Image ноды с путями к каждый текстуре внутри tp ноды',
        script_callback='execfile("%s/createEmptyShader.py")' % PATH_TO_SHOPSCRIPTS,
        script_callback_language=hou.scriptLanguage.Python,
        is_hidden=True
    )
    grp.append(createEmptyShaderP)
    listImagesP = hou.ButtonParmTemplate(
        "list_images",
        "List all texture paths",
        join_with_next=True,
        help='В Python Shell выкидывает список текстур, использованных в каждом шейдере этого шопа',
        script_callback='execfile("%s/listImages.py")' % PATH_TO_SHOPSCRIPTS,
        script_callback_language=hou.scriptLanguage.Python,
    )
    grp.append(listImagesP)
    switchExtensionsEXRP = hou.ButtonParmTemplate(
        "switch_extensions_exr",
        "EXT",
        join_with_next=True,
        help='Заменяет расширение файлов на исходное',
        script_callback='execfile("%s/switchExtensionEXR.py")' % PATH_TO_SHOPSCRIPTS,
        script_callback_language=hou.scriptLanguage.Python,
    )  # , tags={'sourceExt':True})
    grp.append(switchExtensionsEXRP)
    switchExtensionsTXP = hou.ButtonParmTemplate(
        "switch_extensions_tx",
        "TX",
        join_with_next=True,
        help='Заменяет расширение файлов на .tx',
        script_callback='execfile("%s/switchExtensionTX.py")' % PATH_TO_SHOPSCRIPTS,
        script_callback_language=hou.scriptLanguage.Python,
    )  # , tags={'sourceExt':True})
    grp.append(switchExtensionsTXP)
    imagesToAcescgP = hou.ButtonParmTemplate(
        "images_to_acescg",
        "Images to acescg",
        join_with_next=False,
        help='Интерпретировать все текстуры как ACES CG',
        script_callback='execfile("%s/imagesToAcescg.py")' % PATH_TO_SHOPSCRIPTS,
        script_callback_language=hou.scriptLanguage.Python,
        is_hidden=True
    )  # , tags={'sourceExt':True})
    grp.append(imagesToAcescgP)
    bakePathsP = hou.ButtonParmTemplate(
        "bake_paths",
        "Bake Paths",
        join_with_next=True,
        help='Запекает все ссылки текстур и шейдеров в Image нодах. Удобно если нужно удалить эту tp ноду.',
        script_callback='execfile("%s/bakePaths.py")' % PATH_TO_SHOPSCRIPTS,
        script_callback_language=hou.scriptLanguage.Python,
    )
    grp.append(bakePathsP)
    findReplaceP = hou.ButtonParmTemplate(
        "find_replace",
        "Find & Replace",
        join_with_next=True,
        help='Подмена значений в путях на текстуры',
        script_callback='execfile("%s/findAndReplace.py")' % PATH_TO_SHOPSCRIPTS,
        script_callback_language=hou.scriptLanguage.Python,
    )
    grp.append(findReplaceP)
    toacescgP = hou.ButtonParmTemplate(
        "convert_to_acescg",
        "To ACEScg",
        join_with_next=True,
        help='Подмена значений в путях на текстуры',
    ...
    copyTexP = hou.ButtonParmTemplate(
        "copyTex",
        "Copy Textures",
        join_with_next=False,
        help='Копирование файлов',
    ...
    # TEXTURES
    folderP = hou.FolderParmTemplate(
        'textures_folder', 'Textures', folder_type=hou.folderType.Simple
    )
    createParmP = hou.ButtonParmTemplate(
        "create_parm",
        "+",
        join_with_next=True,
        script_callback='execfile("%s/createParm.py")' % PATH_TO_SHOPSCRIPTS,
        script_callback_language=hou.scriptLanguage.Python,
    )
    folderP.addParmTemplate(createParmP)
    removeParmP = hou.ButtonParmTemplate(
        "remove_parm",
        "-",
        join_with_next=True,
        script_callback='execfile("%s/removeParm.py")' % PATH_TO_SHOPSCRIPTS,
        script_callback_language=hou.scriptLanguage.Python,
    )
    folderP.addParmTemplate(removeParmP)
    removeParmsP = hou.ButtonParmTemplate(
        "remove_parms",
        "Remove all texture paths",
        join_with_next=False,
        script_callback='execfile("%s/removeTextureParms.py")' % PATH_TO_SHOPSCRIPTS,
        script_callback_language=hou.scriptLanguage.Python,
    )
    folderP.addParmTemplate(removeParmsP)
    folderP.addParmTemplate(
        hou.SeparatorParmTemplate(name='textures_end', is_hidden=True)
    )
    grp.addParmTemplate(folderP)

    paramsToKeepListP = hou.StringParmTemplate(
        name='parms_to_keep',
        label='Parms to Keep',
        num_components=1,
        default_value=("create_parm remove_parm remove_parms textures_end",),
        is_hidden=True,
    )
...
def collectPathParms(nodes):
    ...
    for n in nodes:
        if (
            n.type().name() != 'arnold_vopnet'
            and n.type().name() != 'arnold_materialbuilder'
        ):
...
def fillExistingTp(tp, nodes):
    ...
    values = []
    toNuke = dict()
    for s, vv in shaders_images.items():
        # print s.name()
        tex_paths = []
        for p, v in vv.items():
    ...
    for s, vv in shaders_images.items():
        for p, v in vv.items():
            ...
            if not grp.find(s.name()):
                grp.insertBefore(
                    grp.find('textures_end'), hou.LabelParmTemplate(s.name(), s.name())
                )
            ...
            print(
                'find is %s'
                % str(grp.find('%s_%s' % (parm_name, str(parm_digit).zfill(2))))
            )
            while grp.find(
                '%s_%s_%s' % (s.name(), parm_name, str(parm_digit).zfill(2))
            ):
                parm_digit += 1
            ...
            # HARDCODE EXCEPTIONS
            if parm_name[0] == '8':
                parm_name = 'EightK_Noise'
            if parm_name[0] == '1':
                parm_name = 'colormask_01'
            ...
            # CREATING PARM
            texP = hou.StringParmTemplate(
                name=parm_name,
                label=parm_name,
                num_components=1,
                default_value=(actual_t,),
                string_type=hou.stringParmType.FileReference,
                tags={'ext': ext, 'node': str(p.path())},
            )
...
def fillExistingTpSet(tp, nodes):
    ...
    for t in val_set:
        ...
        texture_parm_names = [
            p.name()
            for p in folderP.parmTemplates()
            if p.name() and p.name() not in parms_to_avoid
        ]
        ...
        if not match:
            cf = [
                key.node().parm('color_family').eval()
                for (key, value) in ii.items()
                if value == t
            ][0]
            cs = [
                key.node().parm('color_space').eval()
                for (key, value) in ii.items()
                if value == t
            ][0]
            ...
            # CREATING PARM
            parmsLinked = [
                key.path() for (key, value) in list(ii.items()) if value == t
            ]
            texP = hou.StringParmTemplate(
                name=parm_name,
                label=parm_name,
            ...
            listOfKeys = [key for key, value in ii.items() if value == t]  #
            #print('************** LIST OF KEYS %s' % listOfKeys)
            for k in listOfKeys:
                k.set('`chs("../../%s/%s")`' % (tp.name(), parm_name))

            # CREATING COLOR FAMILY PARM
            cfP = hou.StringParmTemplate(
                name='%s_cf' % parm_name,
                label='Family',
                num_components=1,
            ...
            csP.setItemGeneratorScript(
                "__import__('htoa.ocio').ocio.imageColorSpaceMenu(kwargs['node'].parm('%s_cf').eval())"
                % parm_name
            )
            ...
            nodes_to_append = [key.path() for key, value in ii.items() if value == t]
...
def addAssetFolderPathParm(tp):
    if tp.parent().parent() and tp.parent().parent().type().name() == 'subnet':
        ...
        if tp.parm('master_path').rawValue() != '`chs("../../texture_folder")`':
    ...
    if not folderExists:
        for p in button_parms:
            ...
            if not os.path.exists(path):
                ...
                grp.replace(pn, pp)
                #print(
                #    '%s button callback path was replaced by %s'
                #    % (p.name(), '/'.join((pwd, script)))
                #)
...
def main():
    ...
    if len(nodes) == 0:
        ...
        if hou.ui.paneTabOfType(hou.paneTabType.NetworkEditor, 0):
            ...
            if (
                netw_context.type().name() == 'shopnet'
                or netw_context.type().name() == 'matnet'
            ):
                createEmptyTp(netw_context)
            else:
                hou.ui.displayMessage(
                    'Not shopnetw context. Select nodes or execute script in shopnetw context'
                )
                return
        else:
        ...
        nodes[0].type().name() == 'arnold_materialbuilder'
        and nodes[0].name() == 'tp'
        and len(nodes) == 1
    ):
        ...
        nodes[0].type().name() == 'arnold_vopnet'
        or nodes[0].type().name() == 'arnold_materialbuilder'
    ):
