...
def createEmptyTp(parent_shop):
    ...
    masterpathP = hou.StringParmTemplate('master_path', 'Master Path', 1, join_with_next=True, string_type=hou.stringParmType.FileReference)
    ...
    toacescgP = hou.ButtonParmTemplate("convert_to_acescg", "To ACEScg", join_with_next=False,
                                          help=u'Подмена значений в путях на текстуры',
...
def main():
    ...
    if len(nodes) == 0:
        ...
        if hou.ui.paneTabOfType(hou.paneTabType.NetworkEditor, 0):
            ...
            if netw_context.type().name() == 'shopnet':
