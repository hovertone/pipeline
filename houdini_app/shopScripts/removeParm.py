...
existing_texture_names = [i.name() for i in f.parmTemplates() if i.name() not in parms_to_avoid and '_cs' not in i.name() and '_cf' not in i.name() ]
...
for i in res:
    ...
    grp.remove(existing_texture_names[i]+'_cf')
