...
def bakeAnim(node, frameRange, bakeParms = [], parentNode = hou.node('/obj'), byChop = False):
    ...
    if node.type().name() == 'cam':
        bakedNode = parentNode.createNode('cam', node_name = node.name() + "_E")
...
def bakeAnim_ui():
    ...
    if nodes:
        f, l = getRange(nodes[0])
        frameRangeUserInput = hou.ui.readMultiInput('Bake Framerange', ['Start Frame', 'End Frame'], buttons=('OK','Cancel'), initial_contents = [str(f), str(l)])
