...
def importCamHoudini(camFolder, lastFBXVersion, renameTo = 'cam1', scale = '1', popup = True):
    ...
    cam = [i for i in node[0].allSubChildren() if i.type().name() == 'cam'][0]
    print 'CAMH %s' % cam
    ...
    vcom = hou.StringParmTemplate(name='vcomment', label='vcomment', num_components=1)
    parm_group.append(parm_folder)
    parm_group.append(vcom)

    print 'CAM path %s' % cam.path()
    ...
    cam.parm('vcomment').set('Frame: $F4  $HIPFILE')
...
def importLastCamVersionHoudini(force=True):
    ...
    print 'CAM FOLDER % s' % camFolder
    lastFBXVersion = p_utils.common_utils.getLastCamVersion(camFolder)
    print 'LAST FBX VERSION %s' % lastFBXVersion
...
def exportOtherShotCamerasHoudini():
    ...
    for i in toExport:
        ...
        fbx_rop.parm('f1').set(str(int(first_frame)-10))
        fbx_rop.parm('f2').deleteAllKeyframes()
        fbx_rop.parm('f2').set(str(int(last_frame)+10))
