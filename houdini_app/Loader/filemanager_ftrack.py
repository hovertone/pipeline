...
class Filemanager(QDialog, Ui_FileManager):
    ...
    def openMenu(self, position):
        ...
        if len(indexes) > 0:
            ...
            while index.parent().isValid():
                index = index.parent()
        menu = QMenu()
        item = self.listWidget_files.itemWidget(self.listWidget_files.currentItem()).text

        if "." in item:
            if item.rsplit(".", 1)[1] in "ma, mb, hip, nk":
                menu.addAction("Open in Subprocess", self.open_in_subProcess)
    ...
    def open_in_subProcess(self):
        #TODO: NOT WORK NOW.
        path = self.listWidget_files.itemWidget(self.listWidget_files.currentItem()).path
        text = self.listWidget_files.itemWidget(self.listWidget_files.currentItem()).text
        full_path = os.path.join(path, text)
        subprocess.Popen(r"C:\python27\python.exe C:\pipeline\wrapper\run\houdiniACES.py")
