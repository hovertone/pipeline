    ...
    def save_new_asset(self):
        ...
        if self.cbox_sequence.currentText() == "assetBuilds":
            if self.path:
                ...
                pkeys = self.path.split(self.cbox_sequence.currentText())[1].split("/")
                print "PKEYS:", len(pkeys)
                path = "/".join([self.path, name, "main/lookdev/"])
                print "path for new asset IF: ", path
            else:
                ...
                print "path for new asset ELSE: ", path
