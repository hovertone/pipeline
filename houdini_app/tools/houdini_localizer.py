...
class HoudiniLocalizer(object):
    ...
    ext_to_type = dict(
        textures=['.bmp', '.jpg', '.jpeg', '.tif', '.gif', '.tiff', '.rgb', '.png', '.exr', '.pic', '.rat', '.hdr', '.tx'],
                ...
                try:
                    ...
                    if isinstance(parm.parmTemplate(), hou.StringParmTemplate):
                        if parm.parmTemplate().stringType() == hou.stringParmType.FileReference:
                            if (parm.eval() != "") and "`chs(" not in parm.unexpandedString():
                                parms.append(parm)
        return parms
    ...
    def new_name(self, path, node):
        ...
        if not os.path.exists(full_path) and '$F' and '<UDIM>' not in base_name:
    ...
    def copy_file_to_project(self, src, dist, base_name):
        ...
        if '$F' and '<UDIM>' not in src:
            ...
            pat = re.sub('<UDIM>', '\\d+', os.path.basename(src))
            src_dir = hou.expandString(os.path.dirname(src))
            f_print = ''
